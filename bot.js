...
const priceBook   = {};
const marketCache = {};  // key: `${tf}-${windowStart}`
let emitFn = () => {};
let logFn  = () => {};
...
// ── Market discovery ──────────────────────────────────────────────────────────
// Uses Gamma path endpoint: GET /events/slug/{slug}
// Verified working per official Polymarket docs.
...
async function findTokensForSlug(slug) {
  // Primary: events path endpoint — returns event with markets[]
  ...
  // Fallback: markets path endpoint
...
function seedFromMarket(mkt, tokens) {
  ...
  } else if (Array.isArray(prices) && prices.length >= 2) {
    const up = parseFloat(prices[0]) || 0;
    const dn = parseFloat(prices[1]) || 0;
...
async function refreshMarkets() {
  for (const { tf, size } of TIMEFRAMES) {
    ...
    if (!found) log(`⚠️  ${tf}: not found yet — will retry`);
  }
}

// ── REST price polling (every 10s as backup) ──────────────────────────────────
async function pollPrices() {
  const tids = [...new Set(
    Object.values(marketCache).flatMap(w => [w.btcUp, w.btcDn, w.ethUp, w.ethDn].filter(Boolean))
  )];
  if (!tids.length) { log('💲 pollPrices: no token IDs yet'); return; }

  log(`💲 Polling prices for ${tids.length} tokens…`);
  let updated = 0;
  await Promise.all(tids.map(async tid => {
    try {
      ...
      const bidData = await br.json();
      const ask = parseFloat(askData.price ?? 0) || 0;
      const bid = parseFloat(bidData.price ?? 0) || 0;
      if (ask > 0 || bid > 0) {
        priceBook[tid] = { bid, ask };
        updated++;
      } else {
        log(`  ⚠️  No price for ${tid.slice(0,12)}… ask=${JSON.stringify(askData)} bid=${JSON.stringify(bidData)}`);
      }
    } catch (e) {
      log(`  ⚠️  Price fetch error ${tid.slice(0,12)}…: ${e.message}`);
    }
  }));
  log(`💲 Updated ${updated}/${tids.length} prices`);
}
...
function checkEntry(w) {
  const [bUA,bDA,eUA,eDA] = [getAsk(w.btcUp),getAsk(w.btcDn),getAsk(w.ethUp),getAsk(w.ethDn)];
  if (![bUA,bDA,eUA,eDA].every(p => p > 0)) return;
  if (state.openTrades.some(t => t.windowStart===w.windowStart && t.tf===w.tf)) return;
  const g1=bUA+eDA-1, g2=eUA+bDA-1;
...
function buildDashboardSnapshot() {
  const windows = Object.values(marketCache).map(w => {
    const [bUA,bDA,eUA,eDA]=[getAsk(w.btcUp),getAsk(w.btcDn),getAsk(w.ethUp),getAsk(w.ethDn)];
    const [bUB,bDB,eUB,eDB]=[getBid(w.btcUp),getBid(w.btcDn),getBid(w.ethUp),getBid(w.ethDn)];
    return {
...
async function start(emit, logEmit) {
  ...
  log('🚀 Polymarket Arb Bot starting… (5m + 15m windows)');
