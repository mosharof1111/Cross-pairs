...
function loadState() {
  try {
    if (fs.existsSync(TRADES_FILE)) {
      ...
      state = { ...state, ...raw };
      if (state.openTrades.length > 0 && !IS_LIVE) {
...
async function initApiCreds() {
  try {
    ...
    if (res2.ok && text2.startsWith('{')) {
      apiCreds = JSON.parse(text2);
      log(`✅ API creds created: ${apiCreds.apiKey}`);
    } else {
      log(`⚠️  API creds failed: ${text2}`);
    }
...
async function placeBatchSellOrders(trades) {
  ...
  try {
    ...
    const text = await res.text();
    log(`✅ Batch sell: ${orders.length} orders | ${text.slice(0,80)}`);
...
async function cancelOrder(orderId) {
  ...
  } catch (e) { log(`⚠️  cancelOrder: ${e.message}`); }
}
...
async function fetchRealBalance() {
  try {
    ...
    const text = await res.text();
    if (!text.startsWith('{')) {
      log(`⚠️  Balance endpoint blocked (geoblock) — using local balance`);
      return;
    }
...
function checkSignals() {
  ...
  for (const marketId of Object.keys(MARKET_ASSETS)) {
    ...
    if (isTrending(asset)) {
      log(`📊 [${marketId}] TRENDING — skip`); continue;
    }
...
async function placeTrade(marketId, w, cws, direction, move, avg, tokenPrice) {
  ...
  if (state.balance < totalCost) {
    log(`💸 [${marketId}] Low balance $${state.balance} need $${totalCost}`); return;
  }
...
async function checkTP() {
  ...
  for (const { trade: t, exitPrice } of toClose) await closeTrade(t, exitPrice, 'TP');
}
...
async function start(emit, logEmit) {
  ...
  log(`🚀 HYDRA ${IS_LIVE ? '🔴 LIVE' : '🟡 DEMO'} — V2 orders — BTC+ETH+SOL+DOGE 5m`);
